 ...
 */
@Slf4j
public class TraderAGroupDialog extends javax.swing.JDialog implements KeyListener {
     ...
     * @param frame
     */
    public TraderAGroupDialog(JFrame frame) {
        super(frame, false);
        initComponents();
        initKeyListener();
        ...
        tblVou.getSelectionModel().addListSelectionListener((ListSelectionEvent e) -> {
            if (e.getValueIsAdjusting()) {
                if (tblVou.getSelectedRow() >= 0) {
                    ...
                    setCategory(tableModel.getGroup(selectRow));
                }
            }
        });
    }
    ...
    private void save() {
        if (isValidEntry()) {
            accountRepo.saveTraderGroup(group).subscribe((t) -> {
