...
public class RetOutVouSearchDialog extends javax.swing.JDialog implements KeyListener {
     ...
     */
    @Autowired
    private RetOutVouSearchTableModel tableModel;
    ...
    private void search() {
        log.info("Search Return Out History.");
        ...
        //
        Mono<ResponseEntity<List<RetOutHis>>> result = webClient
                .post()
                .uri("/retout/get-retout")
                .body(Mono.just(filter), FilterObject.class)
                .retrieve()
                .toEntityList(RetOutHis.class);
        result.subscribe((t) -> {
    ...
    private void calAmount() {
        float ttlAmt = 0.0f;
        List<RetOutHis> listRO = tableModel.getListDetail();
        if (!listRO.isEmpty()) {
            for (RetOutHis ro : listRO) {
                if (!ro.getDeleted()) {
        ...
        txtTotalRecord.setValue(listRO.size());
