...
public class TraderGroupAutoCompleter implements KeyListener, SelectionObserver {
    ...
    private TraderGroupTableModel traderGroupTableModel;
    ...
    public TraderGroupAutoCompleter() {
    }

    public TraderGroupAutoCompleter(JTextComponent comp, List<TraderGroup> list,
            AbstractCellEditor editor, boolean filter) {
        this.textComp = comp;
        this.editor = editor;
        if (filter) {
            TraderGroup loc = new TraderGroup("-", "All");
        ...
        traderGroupTableModel = new TraderGroupTableModel();
        traderGroupTableModel.setListGroup(list);
        table.setModel(traderGroupTableModel);
        table.setSize(50, 50);
    ...
    public final void setGroup(TraderGroup group) {
        this.group = group;
        if (this.group != null) {
            this.textComp.setText(group.getGroupName());
        } else {
    ...
    public void mouseSelect() {
        if (table.getSelectedRow() != -1) {
            group = traderGroupTableModel.getGroup(table.convertRowIndexToModel(
                    table.getSelectedRow()));
            textComp.setText(group.getGroupName());
            if (editor == null) {
                if (selectionObserver != null) {
                    selectionObserver.selected("TraderGroup", group.getGroupName());
                }
            }
        }
    ...
    Action showAction = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            TraderGroupAutoCompleter completer = (TraderGroupAutoCompleter) tf.getClientProperty(AUTOCOMPLETER);
            if (tf.isEnabled()) {
